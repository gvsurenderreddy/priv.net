...
def bind_device(assocs):
    _socket = gevent.socket.socket

    class Socket(_socket):

        def connect(self, *args, **kwargs):
            addr, port = args[0]  # destination (IP, PORT)
            priv = check_private_host(gevent.socket.gethostbyname(addr))
            if not priv:
                pass
            else:
                iface = assocs[0]
                # define SO_BINDTODEVICE 25
                self.setsockopt(gevent.socket.SOL_SOCKET, 25, iface)
...
def main(args=None, workers=None, client=EchoHubClient, worker_kwargs=None):
    gevent.socket.socket = bind_device(config.VIRTUAL_INTERFACES)
